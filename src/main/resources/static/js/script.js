...
//     let currentSlide = 0;
//     let currentPage = 0; // Start from page 0 for fetching news
//     const BASE_URL = "http://localhost:8080/api";
    
//     let isLoading = false; // Ensure `isLoading` is declared globally
...
//     // Setup slider dynamically (without overwriting existing content)
//     async function setupSlider() {
//         const newsData = await fetchNews(`${BASE_URL}/news/home?page=${currentPage}`);
//         if (!newsData.length) return;
...
//                 </div>
//             `;
//             slider.appendChild(slide);  // Append to the existing slider
//         });
...
//     // Update slider visuals (move to the next slide)
//     function updateSlider() {
//         slider.style.transform = `translateX(-${currentSlide * 100}%)`;
//     }
...
//         console.log("Scroll near bottom detected");
//         if (!isLoading) {
//             console.log("Fetching more news...");
//             isLoading = true;
//             currentPage++;
//             await fetchNewsByPage(currentPage);
//             isLoading = false;
//         } else {
//             console.log("Currently loading, skipping request.");
//         }
//     }
// });

// // Fetch news by page for infinite scroll
// async function fetchNewsByPage(currentPage) {
//     try {
//         const data = await fetchNews(`${BASE_URL}/news/home?page=${currentPage}`);
...
//                     </div>
//                 `;
//                 slider.appendChild(slide);  // Append to existing slides
//             });

//             addClickEventToTitles();  // Add click events for each title
//             slider.children[0].classList.add('active');  // Set the first slide as active
//             updateSlider();  // Update slider visuals (if needed)
//         } else {
